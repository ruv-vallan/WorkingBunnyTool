  ...
  return (
    <div className="p-8 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="card-thermal p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400">진행 중인 프로젝트</p>
              <p className="text-3xl font-bold text-white">{projects.length}</p>
            </div>
            <div className="w-12 h-12 bg-gradient-to-br from-primary-500/20 to-accent-cyan/20 rounded-lg flex items-center justify-center">
        ...
        <div className="card-thermal p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400">오늘 일정</p>
              <p className="text-3xl font-bold text-white">{todayEvents.length}</p>
            </div>
            <div className="w-12 h-12 bg-gradient-to-br from-accent-cyan/20 to-accent-lime/20 rounded-lg flex items-center justify-center">
        ...
        <div className="card-thermal p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400">팀원</p>
              <p className="text-3xl font-bold text-white">{users.length}</p>
            </div>
            <div className="w-12 h-12 bg-gradient-to-br from-accent-purple/20 to-primary-500/20 rounded-lg flex items-center justify-center">
        ...
        <div className="card-thermal p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400">작업 완료율</p>
              <p className="text-3xl font-bold text-white">{progressPercent}%</p>
            </div>
            <div className="w-12 h-12 bg-gradient-to-br from-accent-orange/20 to-thermal-hot/20 rounded-lg flex items-center justify-center">
              <TrendingUp className="w-6 h-6 text-accent-orange" />
            </div>
          </div>
          <div className="mt-4 w-full bg-dark-bg rounded-full h-2 overflow-hidden">
            <div
              className="bg-gradient-to-r from-primary-500 to-accent-orange h-2 rounded-full transition-all shadow-glow-pink"
            ...
            <ul className="space-y-3">
              {myTasks.slice(0, 5).map((task) => {
                ...
                return (
                  <li
                    key={task.id}
                    className="flex items-center p-3 bg-dark-bg/50 rounded-lg hover:bg-sidebar-hover transition-all border border-dark-border"
                  >
                    <div
                      className="w-2 h-2 rounded-full mr-3 shadow-lg"
                      style={{ backgroundColor: project?.color || '#ccc', boxShadow: `0 0 8px ${project?.color || '#ccc'}` }}
                    />
